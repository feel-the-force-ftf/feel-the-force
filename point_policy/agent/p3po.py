...
class BCAgent:
    def __init__(
        ...
        # observation params
        self._obs_dim = point_dim * self.num_track_points + (1 if self.predict_force else 0)
        self.repr_dim = 512
        ...
        if self.temporal_agg:
            action_dim = (self._act_dim + (1 if self.predict_force else 0)) * self.num_queries
        else:
            action_dim = self._act_dim + (1 if self.predict_force else 0)
        ...
        if self.temporal_agg:
            self.all_time_actions = torch.zeros(
                [
                    ...
                    self._act_dim + (1 if self.predict_force else 0),
                ]
            ).to(self.device)
    ...
    def act(self, obs, norm_stats, step, global_step, eval_mode=False, **kwargs):
        ...
        if self.predict_force:
            past_force_states = past_force_states.repeat(past_tracks.shape[0], 1)
            if self.mask_force:
        ...
        if self.temporal_agg:
            action = action.view(-1, self.num_queries, self._act_dim + (1 if self.predict_force else 0))
            ...
            k = 0.01 # reduce to make new action incorporation faster
            ...
            if norm_stats is not None:
                ...
                if self.predict_force:
                    force = post_process["force_states"](action[-1:])
                    action = post_process["actions"](action[:-1])
                    return {'action': action, 'future_force_states': force}
                return {'action': post_process["actions"](action)}
            return {'action': action.cpu().numpy()[0]}
        else:
            if norm_stats is not None:
                ...
                if self.predict_force:
                    ...
                    return {'action': action, 'future_force_states': force}
                return {'action': post_process["actions"](action)}
            return {'action': action.cpu().numpy()[0, -1, :]}
    ...
    def update(self, expert_replay_iter, step, **kwargs):
        ...
        if self.predict_force:
            ...
            action = torch.cat([action, future_force_states[:, :, :, None]], dim=-1)
