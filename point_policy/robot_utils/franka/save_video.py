...
DATA_DIR = Path("path/to/baton-force/Franka-Teach/data/processed_data_pkl/expert_demos/franka_env")
TASK_NAME = "0417_pick_bread_on_plate_teleop"
plot_pts = True
...
lens = [len(data['observations'][i]['gripper_states']) for i in range(len(data['observations']))]
print(lens)
...
for traj_idx in traj_indices:
    ...
    for pixel_key in pixel_keys:
        ...
        if plot_pts and pixel_key != "pixels51":
            ...
            if object_tracks.ndim == 2:
                object_tracks = np.expand_dims(object_tracks, axis=1)
