...
from utils import camera2pixelkey, rigid_transform_3D, filter_and_interpolate_fingertips, sliding_window_outlier_filter
...
for obs_idx, observation in enumerate(DATA["observations"]):
    ...
    # Preserve sensor data if it exists
...
# Preserve sensor normalization data if it exists
...
if args.smooth_robot_tracks:
    # Smooth 3D robot tracks for each camera
    for cam_idx in camera_indices:
        camera_name = f"cam_{cam_idx}"
        pixel_key = camera2pixelkey[camera_name]
