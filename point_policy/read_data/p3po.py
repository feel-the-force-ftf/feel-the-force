...
class BCDataset(IterableDataset):
    def __init__(
        ...
        for _path_idx in self._paths:
            ...
            # Remove stall frames from each demo
            for i, observation in enumerate(data["observations"]):
                # Get robot tracks for pixels1
                robot_tracks = observation["robot_tracks_pixels1"]
                
                # Calculate norm of difference between each frame and first frame
                first_frame = robot_tracks[0]
                diffs = robot_tracks - first_frame  # Shape: (num_frames, num_points, 2)
                norms = np.linalg.norm(diffs, axis=2)  # Shape: (num_frames, num_points)
                max_norms = np.max(norms, axis=1)  # Take max norm across all points
                
                # Find first frame where norm exceeds threshold
                threshold = 0.1
                first_moving_frame = np.argmax(max_norms > threshold)
                
                if first_moving_frame > 0:
                    print(f"Removing {first_moving_frame} stall frames from demo {i}")
                    
                    # Remove stall frames from all relevant fields
                    for key in observation.keys():
                        if isinstance(observation[key], np.ndarray) and len(observation[key]) == len(robot_tracks):
                            observation[key] = observation[key][first_moving_frame:]
                        elif isinstance(observation[key], list) and len(observation[key]) == len(robot_tracks):
                            observation[key] = observation[key][first_moving_frame:]
            
            if binarize_gripper_data:
                for demo in data["observations"]:
                    gripper_states = demo["gripper_states"]
                    new_gripper_states = np.where(gripper_states != -1, 1, -1)
                    demo["gripper_states"] = new_gripper_states
    ...
    def _sample(self):
        ...
        if self._predict_force:
            ...
            future_force_states = future_force_states[:, None]

            if self._temporal_agg and True:
                future_force_state = np.zeros((self._history_len, self._num_queries, 1))
                force = np.zeros((num_actions, 1))
                force[: min(len(future_force_states), sample_idx + num_actions) - sample_idx] = future_force_states[
                    sample_idx : sample_idx + num_actions
                ]
                if len(future_force_states) < sample_idx + num_actions:
                    force[len(future_force_states) - sample_idx :] = future_force_states[-1]
                future_force_state = np.lib.stride_tricks.sliding_window_view(
                    force, (self._num_queries, 1)
                )
                future_force_state = future_force_state[:, 0]
                future_force_state = future_force_state[:, :, 0]
            else:
                future_force_state = future_force_states[
        ...
        if self._predict_force:
            return_dict.update({
                ...
                "future_force_states": self.preprocess["force_states"](future_force_state),
            })
